...
class _InvoiceDetailPageState extends State<InvoiceDetailPage> {
  // 1️⃣ 기존에 등록된 InvoiceController 연결
  final InvoiceController _invoiceController = Get.find<InvoiceController>();

  late String billingId;
  late bool isPaidMode;

  @override
  void initState() {
    super.initState();

    // 2️⃣ Get.arguments 파라미터 안전하게 추출
    final Map<String, dynamic> args = Get.arguments ?? {"isPaidMode": false, "billingId": ""};
    isPaidMode = args["isPaidMode"] ?? false;
    billingId = args["billingId"] ?? "";
    print("billingId-----------$billingId");
    // 3️⃣ [수정] 화면 진입 시점에 preparePaymentGate API 가동하여 데이터 로드
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (billingId.isNotEmpty) {
        _invoiceController.preparePaymentGate(billingId);
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: Obx(() {
          ...
          // 🚨 [수정] selectedDetail 대신 activePaymentMeta 데이터를 관측합니다.
          final paymentData = _invoiceController.activePaymentMeta.value;
          if (paymentData == null) {
            return const Center(
              child: Text("결제 상세 정보를 불러오지 못했습니다.", style: TextStyle(color: Colors.grey, fontSize: 15)),
            );
          }

          // 💡 [수정] 동적 토스 결제용 대표명 추출 로직 (items가 있을 시 첫 품목 외 N건 처리)
          final String orderName = paymentData.items.isNotEmpty
              ? (paymentData.items.length > 1
                    ? "${paymentData.items[0].itemName} 외 ${paymentData.items.length - 1}건"
                    : paymentData.items[0].itemName)
              : "조리원 청구 비용";
          print("paymentData.billingId-----------${paymentData.billingId}");
          return Column(
            children: [
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow("청구번호", paymentData.billingId),
                      _buildInfoRow("계약코드", paymentData.contractId),
                      const SizedBox(height: 24),

                      _buildSectionTitle("청구 항목"),
                      // 💡 [수정] 실제 BillingItem 리스트를 받아 동적 테이블 테이블 뷰를 렌더링합니다.
                      _buildDynamicTable(paymentData.items, formatter),
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Text(
                                  formatter.format(paymentData.totAmt),
                      ...
                      _buildInfoRow("이름", paymentData.custName),
                      _buildInfoRow("연락처", paymentData.custMobile.isNotEmpty ? paymentData.custMobile : "-"),
                      _buildInfoRow("이메일", paymentData.custEmail.isNotEmpty ? paymentData.custEmail : "-"),
                    ],
                  ),
                ),
              ),

              // 하단 고정 버튼 영역 (안드로이드 네비게이션 바 가림 대책 유지)
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      if (isPaidMode) {
                        print("▶️ 매출전표 페이지 라우팅 패스");
                        Get.toNamed("/receiptDetailPage", arguments: {"billingId": paymentData.billingId});
                      } else {
                        print("▶️ 결제 요청 페이지 라우팅 패스");
                        Get.toNamed(
                          "/paymentPage",
                          arguments: {
                            "amount": paymentData.totAmt,
                            "orderName": orderName,
  ...
  // 💡 [수정] BillingItem 다중 배열 리스트를 수용하는 동적 테이블 빌더 함수
  Widget _buildDynamicTable(List<BillingItem> items, NumberFormat formatter) {
    return Container(
      decoration: BoxDecoration(
        border: Border.all(color: const Color(0xFFEFEFEF)),
        borderRadius: BorderRadius.circular(10),
      ...
      child: Table(
        columnWidths: const {0: FlexColumnWidth(1.5), 1: FlexColumnWidth(4), 2: FlexColumnWidth(3.5)},
        border: TableBorder.all(color: const Color(0xFFEFEFEF), width: 1),
        children: [
          _buildTableHeaderRow(),
          // items 리스트의 길이만큼 행(TableRow)을 실시간으로 루프 생성합니다.
          ...items.asMap().entries.map((entry) {
            int index = entry.key + 1; // 순번 표시 (1부터 시작)
            BillingItem item = entry.value;

            return TableRow(
              children: [
                _buildTableCell(index.toString(), isCenter: true),
                _buildTableCell(item.itemName, isCenter: true),
                _buildTableCell(formatter.format(item.payAmt), isCenter: true),
              ],
            );
          }),
        ],
